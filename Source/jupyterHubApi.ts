...
export async function generateNewApiToken(
	...
	try {
		...
		traceError(
			await getResponseErrorMessageToThrowOrLog(
				`Failed to generate token, trying old way`,
				response,
			),
			ex,
		);
...
export async function getUserJupyterUrl(
	...
	let usersJupyterUrl = await getUserInfo(
		baseUrl,
		username,
		token,
		fetch,
...
async function getResponseErrorMessageToThrowOrLog(
	message: string,
	response?: Response,
) {
	if (!response) {
	...
	try {
		...
		traceError(
			`Error fetching text from response to log error ${message}`,
			ex,
		);
	}
...
export async function createServerConnectSettings(
	...
	baseUrl = await getUserJupyterUrl(
		baseUrl,
		authInfo.username,
		authInfo.token,
		fetch,
...
export async function getJupyterHubBaseUrl(
	...
	if (url.toLowerCase().includes("/user/")) {
		try {
			const strippedUrl = url.substring(
				0,

...
export function sendJupyterHubUrlAdded(
	baseUrl: string,
	version: string,
	serverId: string,
) {
	...
	Promise.all([
		...
		.then(([hostNameHash, baseUrlHash]) => {
			publicLog2<JupyterHubUrlAddedData, JupyterHubUrlDataClassification>(
				"addJupyterHubUrl",
				{
					serverId,
					hostNameHash,
...
export function sendJupyterHubUrlNotAdded(
	...
	publicLog2<JupyterHubUrlNotAdded, JupyterHubUrlNotAddedClassification>(
		"addJupyterHubUrl",
		{
			failed: true,
			reason,
...
export function trackUsageOfOldApiGeneration(baseUrl: string) {
	Promise.all([
		...
		.then(([hostNameHash, baseUrlHash]) => {
			publicLog2<
				JupyterHubTokenGeneratedUsingOldAPIData,
				JupyterHubTokenGeneratedUsingOldAPIDataClassification
...
export function solveCertificateProblem(
	problem: "self-signed" | "expired",
	solution: "allow" | "cancel",
) {
	publicLog2<
		JupyterHubUrlCertProblemsSolutionData,
		JupyterHubUrlCertProblemsSolutionDataClassification

...
const isCI =
	process.env.TF_BUILD !== undefined || process.env.GITHUB_ACTIONS === "true";
...
gulp.task("clean:cleanExceptTests", () =>
	del(["clean:vsix", "dist", "out", "!out/test"]),
);
...
gulp.task("checkNpmDependencies", (done) => {
	...
	function checkPackageVersions(packages, parent) {
		...
		expectedVersions.forEach((expectedVersion) => {
			...
			const version =
				packages[expectedVersion.name].version ||
				packages[expectedVersion.name];
			if (!version) {
				return;
			}
...
gulp.task("compile-webextension", async () => {
	...
	if (
		common.getBundleConfiguration() === common.bundleConfiguration.desktop
	) {
		return;
	}
	await buildWebPackForDevOrProduction(
...
gulp.task("webpack-extension-node", async () => {
	...
	await buildWebPackForDevOrProduction(
		"./build/webpack/webpack.extension.node.config.js",
		"extension",
	);
});
...
async function updateBuildNumber() {
	...
	const buildNumber = `${versionParts[2].substring(
		0,
		3,
	)}${buildNumberSuffix}`;
	const newVersion =
...
gulp.task(
	"prePublishBundle",
	gulp.series("webpack", "updatePackageJsonForBundle"),
);
gulp.task(
	"checkDependencies",
...
gulp.task("verifyUnhandledErrors", async () => {
	...
	const contents = fs.pathExistsSync(fileName)
